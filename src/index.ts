...
declare const global: {
  kengineLambdaFlush: () => Promise<void>;
}
...
let coldstart = true;
const tracer = trace.getTracer('@khulnasoft/kengine-lambda-wrapper', '1');
...
export function wrap(handler: Handler) {
  return async function (event: any, lambda_context: Context, callback?: Callback) {
    ...
    try {
      ...
      if (global.kengineLambdaFlush) {

        process.env.DEBUG && console.time('Kengine Trace Export');
        await global.kengineLambdaFlush();
        process.env.DEBUG && console.timeEnd('Kengine Trace Export');
      }
    }
  }
}
